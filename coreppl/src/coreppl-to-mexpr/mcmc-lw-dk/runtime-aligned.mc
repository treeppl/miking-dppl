...
let moveSample: all a. use RuntimeDistBase in Dist a -> (Any, Float) = 
  ...
  let prevSample = deref state.preValue in
  ...
  modref state.qdk (subf logLikProp logLikPrev);
    ...
    lam i. lam samples. lam acc.
      match samples with [sample] ++ samples then
        ...
        let acc: [Option (Any, Float)] =
          cons (if eqi i 0 then modref state.preValue sample; None () else Some sample) acc in
